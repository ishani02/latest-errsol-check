...
function DebugAPI(connection, debuggerSessionId, forkedProcess, executeCodeRestricted) {
  var self = this;
  ...
  this._executeCodeRestricted = executeCodeRestricted;
  ...
  this._forkedProcess.on('exit', (code) => {
    ...
    try {
      Server.call('com.errsole.web.debugger_session.'+debuggerSessionId+'.stop_debugger', [], {}).catch(console.error);
    } catch(e) {
      return e;
    }
  });
...
DebugAPI.prototype.initialize = function () {
  ...
  try {
    this._forkedProcess.on('message', function(data) {
      var msg = (data && data.data) || {};
      if(msg.type === 'uncaught_exception_occurred') {
        var debuggerSessionId = self._debuggerSessionId;
        Server.publish('com.errsole.module.debugger_session.'+debuggerSessionId+'.uncaught_exception_occurred', [], {});
      }
    });
  } catch(e) {
    return e;
...
DebugAPI.prototype.updateScriptSource = async function (args, kwargs, details) {
  ...
  if(self._executeCodeRestricted) {
    return {
      'error': 'Code editing is disabled.',
  ...
  try {
    ...
    var newScript = await this._connection.send('Debugger.getScriptSource', data);
    if(result.exceptionDetails) {
      return result;
    } else if(newScript.scriptSource !== data.scriptSource) {
      return {
        'reason': 'not_modified',
        'result': result
      }
...
DebugAPI.prototype.postRequest = async function (args, kwargs, details) {
  ...
  try {
    await this._connection.send('Debugger.setSkipAllPauses', {skip: false});
...
DebugAPI.prototype.cancelRequest = async function (args, kwargs, details) {
  var fork = activeForks.getFork(this._debuggerSessionId);
  try {
    var result = await fork.Http.cancelRequest(kwargs, this._debuggerSessionId);
    if(result) {
      await this._connection.send('Debugger.setSkipAllPauses', {skip: true});
      await this._connection.send('Debugger.resume', {terminateOnResume: false});
      return true;
    } else {
      return false;
    }
...
DebugAPI.prototype.RuntimeConsoleEvaluate = async function (args, kwargs, details) {
  ...
  if(self._executeCodeRestricted) {
    return {
      'error': "JavaScript expression evaluation is disabled.",
