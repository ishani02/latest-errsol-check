...
ForksManager.initialize = function() {
  ...
  Events.on(Events.ROUTER_CONNECTION_ESTABLISHED, function() {
    Server.register(Server.START_DEBUGGER_URI, function(args, kwargs, details) {
      ...
      var executeCodeRestricted = kwargs.executeCodeRestricted || false;
      ...
      return getPorts(wrappedPorts.length + 1).then(async function(ports) {
        ...
        forkOptions.executeCodeRestricted = executeCodeRestricted;
        ...
        if(isForkProcessStarted == true) {
          ...
          var options = appConfig.getOptions();
          try {
            var requireCache = Object.keys(require.cache);
            var processModuleList = Object.keys(process.moduleLoadList);
            options['totalScriptParsed'] = requireCache.length + processModuleList.length;
          } catch(e) {
            console.error(e);
          }
          return {
